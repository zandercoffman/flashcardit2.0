...
import { AudioWaveform, Bomb, BookCheck, BookOpen, Ellipsis, Image, LandPlot, LayoutTemplate, Megaphone, Milestone, Music, PackageOpen, PencilLine, Settings, Settings2, Sparkles, Volume, Volume2 } from "lucide-react";
          ...
          currentPage == "set" && !isMobile && <>
            <div className="flex flex-row gap-2">
              <Select value={currentMode || "normal"} onValueChange={setCurrentMode}>
                <SelectTrigger className="flex flex-row items-center justify-center overflow-hidden w-full rounded-xl bg-background/50 hover:bg-background/70 transition-colors">
...

                    {
                      currentSetId !== undefined && isSetInAnyList(currentSetId) && <SelectItem value="resources">
                        <div className="flex justify-start items-center gap-4 ">
                          ...
                          <div className=" w-full">
                            <h4 className="text-xl md:text-base font-semibold flex flex-col ">Resources <Badge variant={"outline"}>for {getShortNameFromSetFromId(currentSetId ?? "")}</Badge></h4>
                          </div>
                        </div>
                      </SelectItem>
                    }
