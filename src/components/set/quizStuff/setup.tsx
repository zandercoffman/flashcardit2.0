...
export function QuizSetup({ onStartQuiz }: QuizSetupProps) {
  ...
  return (
    <div className="flex-1 flex flex-col w-full">
      <Card className="relative overflow-hidden w-[90vw] mx-auto md:w-[70vw] p-8 mb-8 flex flex-col lg:flex-row rounded-3xl border border-[#2ED4C4]/30 light:border-slate-300 bg-[#0E1424]/80 light:bg-white/95 backdrop-blur-xl text-white light:text-slate-900">
        ...
        <div className="flex flex-col w-full lg:w-1/2">
          <div className="text-center mb-8">
            ...
            <h2 className="font-bold text-2xl mb-2 text-white light:text-slate-900">Choose your challenge</h2>
            <p className="text-muted-foreground light:text-slate-600 text-sm">Stack modes to fit your energy level today.</p>
          </div>
          {/* Preview of selected modes */}
          <div className="p-4 mt-auto bg-white/5 light:bg-slate-100 rounded-2xl border border-white/10 light:border-slate-300">
            <h4 className="font-medium text-sm mb-2 text-white light:text-slate-900">Selected Features</h4>
            <div className="flex flex-wrap gap-2 text-white light:text-slate-900">
              {streakMode && (
              ...
              {!streakMode && !timerMode && !randomMode && <span className="text-muted-foreground light:text-slate-600 text-xs">Standard quiz mode</span>}
            </div>
          </div>
        ...
        <div className="flex flex-col gap-10 w-full lg:w-1/2 lg:mt-auto justify-center items-center lg:justify-end lg:items-end">
            ...
            <Card
              className={cn(
                ...
                streakMode
                  ...
                  : "border-white/15 light:border-slate-300 bg-white/5 light:bg-white",
              )}
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-medium text-white light:text-slate-900">Streak Mode</h3>
                    <p className="text-sm text-muted-foreground light:text-slate-600">
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-medium text-white light:text-slate-900">Timer Challenge</h3>
                    <p className="text-sm text-muted-foreground light:text-slate-600">
                      Answer quickly to earn bonus points and increase the challenge
                    </p>
                    {timerMode && (
                      <div className="mt-2" onClick={(e) => e.stopPropagation()}>
                        <label className="text-xs text-muted-foreground light:text-slate-600 mr-2">Seconds</label>
                        <select
                          value={timerSeconds}
                          onChange={(e) => setTimerSeconds(Math.min(10, Math.max(5, Number(e.target.value) || 5)))}
                          className="h-8 rounded-md border border-white/20 light:border-slate-300 bg-white/5 light:bg-white px-2 text-xs text-white light:text-slate-900"
                        >
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-medium text-white light:text-slate-900">Random Sort of Questions</h3>
                    <p className="text-sm text-muted-foreground light:text-slate-600">
          ...
          <InteractiveHoverButton onClick={handleStartQuiz} className="cursor-pointer">
            <span className="flex flex-row justify-center items-center text-white light:text-slate-900">
      ...
      <div className="text-center pb-8">
        <span className="text-muted-foreground light:text-slate-600 text-sm">Adaptive, streak-ready, and timing-aware.</span>
      </div>
    </div>
  )
}
